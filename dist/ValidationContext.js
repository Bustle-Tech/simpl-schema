...
function () {
  ...
  function ValidationContext(ss, name) {
    ...
    if (tracker) {
      var addDeps = function addDeps(schema) {
        var keys = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];
        Object.keys(schema).forEach(function (key) {
          var schemaElement = schema[key];
          schemaElement.type.definitions && schemaElement.type.definitions.forEach(function (def) {
            if (_typeof(def.type) === 'object') {
              addDeps(def.type._schema, keys.concat(key));
            } else {
              var fullKey = keys.concat(key).join('.');
              _this._deps[fullKey] = new tracker.Dependency();
            }
          });
        });
      };
